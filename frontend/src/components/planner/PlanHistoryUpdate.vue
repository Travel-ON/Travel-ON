  ...
  <div style="flex: 6">
    <div style="height: 90px; display: flex; align-items: center">
      <div style="display: flex; align-items: center; margin-left: 20px">
        <div style="margin-right: 10px"><v-icon>mdi-cached</v-icon></div>
        <div style="font-weight: bold; font-size: 28px">방문장소 수정</div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    axios({
      url: spring.plan.gugun(this.plan.sidoName),
      method: "get",
    })
      .then((res) => {
        console.log("mounted");
        console.log(this.plan.sidoName);
        console.log(res);
        this.gugun = res.data;
      })
      .catch((err) => {
    ...
    this.planData.sidoName = this.plan.sidoName;
    this.planData.gugunName = this.plan.gugunName;
    this.planData.visitDate = this.plan.visitDate;
    this.selectedSido = this.plan.sidoName;
  },
  methods: {
    ...mapGetters(["token"]),
