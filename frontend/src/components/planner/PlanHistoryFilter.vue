...
export default {
  data: () => ({
    valid: true,
    filterData: {
      startDate: new Date(year, month, day - 7).toISOString().substr(0, 10), // 일주일 전 으로 초기화,
  ...
  watch: {
    dateState(val) {
      if (val === 0) {
        this.filterData.startDate = new Date(year, month, day - 7).toISOString().substr(0, 10);
        this.isDisabled = true;
      } else if (val === 1) {
        this.filterData.startDate = new Date(year, month - 1, day).toISOString().substr(0, 10);
        this.isDisabled = true;
      } else if (val === 2) {
        this.filterData.startDate = new Date(year, month - 3, day).toISOString().substr(0, 10);
        this.isDisabled = true;
      } else if (val === 3) {
        this.filterData.startDate = new Date(year, month - 6, day).toISOString().substr(0, 10);
        this.isDisabled = true;
      } else {
